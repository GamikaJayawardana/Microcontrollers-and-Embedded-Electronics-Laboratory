...
void main() {
    ...
    while (1) {
        if (sw1 == 0 && last_state == 1) {  // Detect button press (falling edge)
            ...
            Delay_ms(50);       // 50ms debounce delay
        }
        else if(sw1 == 1) {     // Button is released
            last_state = 1;     // Reset to not pressed state
        }
    }
}
